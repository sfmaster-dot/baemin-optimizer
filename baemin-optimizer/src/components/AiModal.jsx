...
// ── 캐시 키 결정 함수 ──
// menuname/menudesc/menuoption은 메뉴명별로 분리, 나머지는 type 그대로
// reply는 일회성이라 null 반환 (저장 ❌)
function getCacheKey(type, form) {
  ...
  if (type === 'menuoption') {
    if (form.mode === 'board') return 'menuoption:board';
    const name = (form.menuName || '').trim();
    return name ? `menuoption:${name}` : null;
  }
  return type; // intro, notice, orderguide
}
...
export default function AiModal({ item, onClose, userId, storeId }) {
  ...
  // 저장 가능 여부 (reply·미로그인 매장 ❌)
  const canSave = !!(userId && storeId && type !== 'reply');

  // 현재 cacheKey 계산
  const cacheKey = useMemo(() => getCacheKey(type, form), [type, form.currentName, form.menuName, form.mode]);
